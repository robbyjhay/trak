...
export function EditResourceModal({
  ...
  open: boolean;
  onClose: () => void;
  onUpdated: (resource: LibraryResource) => void;
}) {
  ...
  async function handleSubmit() {
    ...
    setFormError("");
    try {
      const res = await apiSend<{ resource: LibraryResource }>(`/api/library/${resource.id}`, "PUT", {
        title: cleanTitle,
        category,
      ...
      showToast("Resource updated", "Your changes have been saved.");
      onUpdated(res.resource);
      onClose();
    } catch (err) {
      setFormError(err instanceof Error ? err.message : "Could not update. Try again.");
      setSubmitting(false);
    }
  }

  const shownPreview = previewUrl || youtubeThumb || resource?.thumbnailUrl;

  return (
    <ModalBackdrop open={open} onClose={onClose} labelledBy="edit-resource-title" describedBy="edit-resource-desc" bottomSheetOnMobile>
      <ModalPanel wide bottomSheetOnMobile className="sm:w-[900px] sm:max-w-[95vw]">
        ...
        <div className="mb-5 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="edit-resource-title" className="font-display text-[20px] font-bold text-foreground">
              Edit resource
            </h2>
            <p id="edit-resource-desc" className="text-[12.5px] text-foreground-secondary">
              Update the details for this Library resource.
            </p>
          </div>
        </div>
        ...
        >
          <div>
            <label htmlFor="edit-lib-title" className="mb-1.5 block text-[12.5px] font-bold text-foreground">
          ...
          {/* Type — dropdown */}
          <div>
            <label htmlFor="edit-lib-type" className="mb-1.5 block text-[12.5px] font-bold text-foreground">
          ...
          <div>
            <label htmlFor="edit-lib-desc" className="mb-1.5 block text-[12.5px] font-bold text-foreground">
          ...
          <div>
            <label htmlFor="edit-lib-url" className="mb-1.5 block text-[12.5px] font-bold text-foreground">
            ...
            <input
              ...
              aria-describedby={urlInvalid ? undefined : "edit-lib-url-hint"}
            />
            {urlMissing ? (
              <p className="mt-1 text-[11.5px] text-critical-semantic">
              ...
              <p id="edit-lib-url-hint" className="mt-1 text-[11.5px] text-foreground-faint">
                {isYouTube
